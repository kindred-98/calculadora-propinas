...
def mock_widgets():
    ...
    with patch('src.gui.ttk.Frame') as mock_frame, \
        ...
        # CORRECCIÓN: cada llamada a ttk.Entry() devuelve una instancia distinta
        # para que gui.monto_entry, gui.valor_entry y gui.personas_entry
        # sean objetos independientes y se puedan mockear por separado.
        def make_entry_instance(*args, **kwargs):
            instance = Mock()
            instance.pack = Mock()
            instance.get = Mock(return_value="0")
            return instance

        mock_entry.side_effect = make_entry_instance
...
def test_crear_widgets_crea_todos_los_componentes(mock_tk_root, mock_widgets):
    ...
    - Gráfico (matplotlib)
    """
    _gui = TipCalculatorGUI(mock_tk_root)
...
def test_calcular_con_propina_porcentaje(mock_formatear, mock_dividir, mock_tk_root, mock_widgets):
    ...
    # Configurar mocks de entrada — ahora cada atributo es una instancia distinta
...
def test_manejar_valor_error_de_entrada_invalida(mock_dividir, mock_showerror, mock_tk_root, mock_widgets):
    ...
    # Configurar entrada inválida — instancias independientes gracias al fixture corregido
